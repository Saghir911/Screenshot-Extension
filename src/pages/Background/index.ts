...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.action === "beginCapture" && msg.tabId) {
    // msg.tabId may be number | undefined depending on what the caller sent
    targetTabId = msg.tabId as number;
    isCapturing = true;

    // Try to start the content script on that tab (guarded)
  ...
  if (msg.action === "capture") {
    // Determine tab id from sender, message or stored targetTabId
    const maybeTabId = sender.tab?.id ?? msg.tabId ?? targetTabId;
    if (typeof maybeTabId !== "number") {
      // no valid tab id -> ignore
      sendResponse({ status: "no-valid-tab" });
      return true;
    }
    const tabId = maybeTabId; // now narrowed to number
...
function startContentScriptAndBegin(tabId: number) {
  ...
  chrome.tabs.sendMessage(tabId, { action: "startCapturing" }, (res) => {
    if (chrome.runtime.lastError) {
      console.warn("[background] content script not found; injecting...", chrome.runtime.lastError.message);
      ...
      chrome.tabs.captureVisibleTab(
        ...
        (dataUrl) => {
          ...
          chrome.storage.local.set({ screenshotUrl: dataUrl }, () => {
            ...
            chrome.tabs.sendMessage(tabId, { action: "scrollNext", dataURI: dataUrl }, (res) => {
              if (chrome.runtime.lastError) {
                // content script disappeared (tab closed / navigated) -> stop capturing
                console.error("[background] sendMessage(scrollNext) error:", chrome.runtime.lastError.message);
                isCapturing = false;
                targetTabId = null;
                reject(chrome.runtime.lastError);
                return;
